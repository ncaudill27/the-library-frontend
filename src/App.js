...
class App extends Component {
  ...
  currentUsersClubs = () => {
    ...
    const membershipAssociations = memberships.filter(
      m => m.userId === currentUser.id
    )
    clubs = membershipAssociations.map(({ clubId }) =>
  ...
  clubsCurrentUserMods = () => {
    let { currentUser, memberships, clubs } = this.props
    const modAssociations = memberships.filter(
      m => m.userId === currentUser.id && m.isMod
    )
    clubs = modAssociations.map(({ clubId }) =>
      clubs.find(c => c.id === clubId)
    )
    return clubs
  }
  ...
  render() {
    const {
      props: { message, currentUser, memberships },
      reifyClubById,
      currentUsersClubs,
      clubsCurrentUserMods,
    } = this
