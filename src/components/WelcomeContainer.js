  ...
  return (
    <div>
      <Typography variant="body1" paragraph>
        Not a member of any clubs yet? Check out the "Clubs" tab from the menu
        on the top left! Here's a shortcut this time.
      </Typography>
      <Link
        href="/clubs"
        underline="none"
...
const SelectBookMessage = () => {
  ...
  return (
    <div>
      <Typography variant="body1" paragraph>
        Also, be sure to browse through the New York Times Bestsellers list and
        mark whichever book you are currently reading. Also found in the menu
        under "Bestsellers"
      </Typography>
      <Link
        href="/bestsellers"
  ...
  return (
    <div>
      ...
      <Paper className={classes.paper} square>
        ...
        <Typography variant="subtitle1" paragraph>
          The current selection of NY Times bestsellers not enough for your
          needs? Soon you will have the full power of Google available!
        </Typography>
        <Typography variant="h6">Polling</Typography>
        <Typography variant="subtitle1" paragraph>
          Club mods, an easier way to vote on your clubs next book will be
          coming shortly! Allowing users to vote for their next read.
        </Typography>
      </Paper>
    </div>
  )
...
const ProperWelcome = ({ currentUser, currentUsersClubs, isbn }) => {
  ...
  return (
    <>
      ...
      {isOnboarded() && (
        <OnboardingMessage isReading={isReading} isMember={isMember} />
      )}
      <ComingSoon />
    </>
  )
