...
function Book({title, author, description, src, isbn13, currentUser, updateUserRequest, patchClubRequest, clubsCurrentUserMods}) {
  ...
  const select = () => (
    <FormControl fullWidth>
      ...
      <Select name='isbn' onChange={setUpdateTarget}>
        <MenuItem value={currentUser.username} className={classes.option}>{currentUser.username}</MenuItem>
  ...
  const clubOptions = () => {
    return clubsCurrentUserMods().map( club => <MenuItem key={club.id} value={club.id} className={classes.option}>{club.name}</MenuItem> );
  }
