  ...
  switch (action.type) {
    ...
    case "ADD_USERS":
      users = action.users.map(user => {
        return {
          ...
          currentlyReading: !!user.attributes.favoriteBookIsbn13
            ? user.attributes.favoriteBookIsbn13.replace(/-/g, "")
            : null,
          commentIds: user.relationships.comments.data.map(
            comment => comment.id
          ),
        }
      })
      ...
      return {
        ...
        memberships: state.memberships.concat(memberships),
        pending: false,
      }
    ...
    case "LOGIN_USER":
      const {
        id,
        attributes: { name, username, email, bio, avatar, favoriteBookIsbn13 },
        relationships: {
      ...
      const currentUser = {
        ...
        currentlyReading: !!favoriteBookIsbn13
          ? favoriteBookIsbn13.replace(/-/g, "")
          : null,
        commentIds: comments.map(comment => comment.id),
      }

      users = state.data.map(user =>
        user.id !== currentUser ? user : currentUser
      )
